...
- Name: contact.spec.ts >> Formulaire de contact >> affiche le formulaire de contact
- Location: e2e\contact.spec.ts:11:7
...
Error: strict mode violation: locator('textarea') resolved to 2 elements:
    1) <textarea rows="4" name="message" placeholder="Décrivez votre projet…" class="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 transition-colors resize-none"></textarea> aka getByRole('textbox', { name: 'Décrivez votre projet…' })
    2) <textarea rows="5" required="" name="message" placeholder="Votre message…" class="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 transition-colors resize-none"></textarea> aka getByRole('textbox', { name: 'Votre message…' })
...
```ts
  ...
  2  | 
  3  | /**
  4  |  * Tests E2E — Formulaire de contact
  5  |  */
  6  | test.describe("Formulaire de contact", () => {
  7  |   test.beforeEach(async ({ page }) => {
  8  |     await page.goto("/#contact");
  9  |   });
  10 | 
  11 |   test("affiche le formulaire de contact", async ({ page }) => {
  12 |     await expect(page.locator("#contact")).toBeVisible();
  13 |     await expect(page.locator('input[name="name"]').last()).toBeVisible();
  14 |     await expect(page.locator('input[name="email"]').last()).toBeVisible();
> 15 |     await expect(page.locator("textarea")).toBeVisible();
     |                                            ^ Error: expect(locator).toBeVisible() failed
  16 |   });
  17 | 
  18 |   test("soumet le formulaire de contact avec données valides", async ({ page }) => {
  19 |     // On cible les champs dans la section contact (les derniers si deux formulaires)
  20 |     const section = page.locator("#contact");
  21 |     await section.locator('input[name="name"]').fill("Test Contact");
  22 |     await section.locator('input[name="email"]').fill("contact@test.com");
  23 |     await section.locator("textarea").fill("Ceci est un message de test Playwright.");
  24 | 
  25 |     await section.locator('button[type="submit"]').click();
  26 | 
  27 |     await expect(
  28 |       page.locator("text=/envoyé|merci|message reçu/i")
